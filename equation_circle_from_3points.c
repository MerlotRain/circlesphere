...
int is_perpendicular(const point2d_t p1, const point2d_t p2,
                     const point2d_t p3) {
...
double calc_circle(const point2d_t p1, const point2d_t p2, const point2d_t p3,
                   circle_t *c) {
  assert(c);
  ...
  // calc center
  c->center.x = (aSlope * bSlope * (p1.y - p3.y) + bSlope * (p1.x + p2.x) -
                 aSlope * (p2.x + p3.x)) /
                (2 * (bSlope - aSlope));
  c->center.y =
      -1 * (c->center.x - (p1.x + p2.x) / 2) / aSlope + (p1.y + p2.y) / 2;

  c->radius = point2d_distance(c->center, p1); // calc. radius
  return c->radius;
}

void equation_circle_from_3point(const point2d_t p1, const point2d_t p2,
                                 const point2d_t p3, circle_t *c) {
  assert(c);
  c->radius = -1;
